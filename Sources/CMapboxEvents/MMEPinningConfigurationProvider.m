...
const NSString *kMMEPinnedDomains = @"kMMEPinnedDomains";
const NSString *kMMEPublicKeyHashes = @"kMMEPublicKeyHashes";
const NSString *kMMEExcludeSubdomainFromParentPolicy = @"kMMEExcludeSubdomainFromParentPolicy";

@interface MMEPinningConfigurationProvider()

@property (nonatomic, readwrite) NSDictionary *pinningConfig;
@property (nonatomic) MMEHashProvider *hashProvider;
...
+ (MMEPinningConfigurationProvider *)pinningConfigProviderWithConfiguration:(MMEEventsConfiguration *)configuration {
    ...
    if (configuration) {
        [pinningConfigurationProvider.hashProvider updateHashesWithConfiguration:configuration];
    }
